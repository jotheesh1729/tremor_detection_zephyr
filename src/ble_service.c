...
static const struct bt_uuid_128 report_chrc_uuid =
...
BT_GATT_SERVICE_DEFINE(tremor_svc, BT_GATT_PRIMARY_SERVICE(&tremor_svc_uuid),
			BT_GATT_CHARACTERISTIC(&report_chrc_uuid.uuid, BT_GATT_CHRC_NOTIFY,
 ...
 *   [0] tremor episode count      [1] dyskinesia episode count
 *   [2:3] tremor duration (s)     [4:5] dyskinesia duration (s)
 *   [6] mean severity (0-100)
 */
void ble_service_notify_report(const struct tremor_report *report)
{
	uint8_t buf[7];

	buf[0] = report->tremor_episodes;
	buf[1] = report->dyskinesia_episodes;
	sys_put_le16(report->tremor_duration_s, &buf[2]);
	sys_put_le16(report->dyskinesia_duration_s, &buf[4]);
	buf[6] = report->mean_severity_pct;

	bt_gatt_notify(NULL, &tremor_svc.attrs[REPORT_ATTR_IDX], buf, sizeof(buf));
}

/* Wire format, 6 bytes, little-endian:
 *   [0] type (1=tremor, 2=dyskinesia)   [1:2] duration (s)
 *   [3:4] dominant frequency (deci-Hz)  [5] mean severity (0-100)
 */
